    ...
    public class Shaker : MonoBehaviour
    {
        ...
        private void ShakeUpdate()
        {
            if (isShaking)
            {
                if (shakeProgress <= 1f)
                {
                    shakeProgress += (1 / s.duration) * Time.deltaTime;
    ...
    public class PostProcesser : MonoBehaviour
    {
        ...
        private void PPUpdate()
        {
            if (isPlaying)
            {
                if (progress <= 1f)
                {
                    progress += Time.deltaTime * (1 / pp.duration);
    ...
    public class MaterialPlayer : MonoBehaviour
    {
        ...
        private void MaterialUpdate()
        {
            if (isPlaying)
            {
                if (progress <= 1f)
                {
                    progress += (1 / m.duration) * Time.unscaledDeltaTime;
    ...
    public class Bouncer : MonoBehaviour
    {
        ...
        private void BounceUpdate()
        {
            if (isPlaying)
            {
                if (progress <= 1f)
                {
                    progress += (1 / b.duration) * Time.deltaTime;
                    for (int i = 0; i < b.bouncedObjects.Length; i++)
                    {
